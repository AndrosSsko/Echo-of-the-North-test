...
# NOTE: is_slipping_in_slime / slime_scramble_timer / weapon_dropped_this_slip
# used to track slip state here directly. That's now handled entirely by
# blackboard["is_slipped"] / ["slip_elapsed"] / ["slip_weapon_dropped"].
...
func _on_distraction_sound_emitted(sound_position: Vector3, hearing_distance: float) -> void:
	# Any guard within hearing_distance of the sound reacts on its own --
	# gadgets no longer need to know about EnemyGroup or investigate_noise().
	if global_position.distance_to(sound_position) <= hearing_distance:
		investigate_noise(sound_position)

func _physics_process(delta: float) -> void:
	
	# === 1. BEHAVIOR TREE: SLIME-SLIP TASK ===
	# Driven entirely by blackboard["is_slipped"] now. current_phase is still
	# set to SLIPPING (see execute_cascade_stumble_fall) purely so the rest
	# of this file's existing PatrolPhase-based checks -- vision blanking,
	# cascade-fall triggers, the mutual-witness radar, etc. -- continue to
	# treat him as incapacitated without having to touch every one of those
	# call sites. The actual scramble / face-plant / recovery behavior now
	# lives entirely in TaskSlimeSlip.
	...
	# NOTE: SLIPPING is deliberately not listed here -- the blackboard-driven
	# slip block above always returns before execution reaches this point.
...
func execute_disarm_parry_drop() -> void:
	...
	# BUGFIX: This function is also called from TaskSlimeSlip (see
	# task_slime_slip.gd -> _begin_faceplant) purely to trigger the weapon-drop
	# visuals during a slime slip. It must NOT hijack current_phase or velocity
	# in that case: doing so used to silently swap SLIPPING -> PANICKING,
	# which broke the slip recovery entirely and left guards stuck face-down
	# forever. It also zeroed out the forward launch impulse the slip
	# sequence had just set, canceling the face-plant slide. Only take over
	# phase/velocity for the ordinary parry-disarm case (see player.gd's
...
func execute_cascade_stumble_fall() -> void:
	...
	# That's it -- no fire-and-forget timers. From here, slip_task.execute_task()
	# drives the scramble -> face-plant -> recovery sequence every physics
	# frame (called from _physics_process above) purely off blackboard state.


## Called by TaskSlimeSlip the instant the scramble ends and the forward
## face-plant launch begins. Movement, weapon disarm, and the mesh tilt
## itself are handled by the task; this hook only covers presentation
## flourishes specific to this enemy type (labels, camera shake, dizzy halo).
...
## Called by TaskSlimeSlip once the 3.0s stun timeout completes and it
## returns SUCCESS. blackboard["is_slipped"] is already false by this point.
func _on_slip_recovered() -> void:
	...
	if is_instance_valid(visual_mesh):
		visual_mesh.position.y = 0.0 # Reset the dip set in _on_slip_faceplant
...
func _on_gadget_impact_received(type: int, impact_pos: Vector3, radius: float) -> void:
	...
	if distance > radius: return # Completely unaffected outside the radius
	
	match type:
		EventBus.GadgetType.SLIME:
			# Instantly triggers your TaskSlimeSlip sequence via blackboard!
			execute_cascade_stumble_fall()
		EventBus.GadgetType.BOLA:
			# Instantly snags his feet and triggers TaskBolaStruggle!
